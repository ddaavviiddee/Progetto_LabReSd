...
augmented_data = RandomOverSampler(sampling_strategy={0: 1000, 1: 1000, 2: 1000}, random_state=42)
...
def knn_classify(X_train, Y_train, X_test, k):
    prediction = []
    for test_point in tqdm(X_test, desc="Classifying"):
        distances = []
        for i, train_point in enumerate(X_train):
            d = euclidean_distance(test_point, train_point)
            print("test", test_point)
            print("train", train_point)
            c = Y_train[i]
            distances.append((c, d))
        
        # Ordina la lista di distanze e prende i primi k elementi
        distances.sort(key=lambda x: x[1])
        k_nearest_neighbors = distances[:k]

        # Trova la classe più comune (moda) tra i vicini
        k_nearest_classes = [neighbor[0] for neighbor in k_nearest_neighbors]
